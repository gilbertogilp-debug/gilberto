...
import { X, Mail, Lock, User as UserIcon, Sparkles, ShieldCheck, ArrowRight, CheckCircle2, Check } from 'lucide-react';

export const AuthModal: React.FC = () => {
  const { isAuthModalOpen, setIsAuthModalOpen, authModalMode, setAuthModalMode, login, registerCustomerAccess, showToast, plans } = useApp();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (authModalMode === 'register') {
      ...
      registerCustomerAccess(name || 'Cliente VIP', email, '', password || '123456', chosenPlan);
      showToast(`🎉 Cadastro no ${chosenPlan} realizado com sucesso!`);
      return;
    }
                ...
                <div className="space-y-1.5 pt-1">
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 flex items-center justify-between">
                    <span className="flex items-center gap-1">
                      ...
                      <span>Escolha o Plano Desejado</span>
                    </span>
                      ...
                      return (
                        <button
                          ...
                          className={`relative p-2 rounded-xl border text-left transition-all cursor-pointer flex flex-col justify-between ${
                            isSelected
                              ? 'bg-gradient-to-br from-purple-500/10 to-blue-500/10 border-purple-500 text-slate-900 dark:text-white ring-2 ring-purple-500/30'
                              : 'bg-slate-50 dark:bg-slate-800/60 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:border-purple-300'
                          }`}
                        >
                          <div>
                            <div className="flex items-center justify-between mb-0.5">
                              <span className="font-extrabold text-[10px] text-slate-900 dark:text-white truncate">
                                {plan.id}
                              </span>
